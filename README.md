...
| PDFリンク → 全キャリア一括更新 | DriveのPDFを探索してリンクシートに反映 |
| PDFリンク → SoftBankのみ / Ymobileのみ | キャリア別にリンク更新 |
| 金額取得・ファイル名更新 → 全キャリア一括更新 | PDFをOCRして金額をファイル名に反映 |
| 金額取得・ファイル名更新 → SoftBankのみ / Ymobileのみ | キャリア別に金額取得 |
...
├── shared_utils.py                 # SoftBank・Ymobile共通ユーティリティ
├── SoftBank/
...
GAS メニュー「携帯領収書管理 ツール」→「金額取得・ファイル名更新」を実行する。Drive APIへのアップロード直後は反映に数秒かかる場合があるのでしばらく待ってから再実行する。
