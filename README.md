...
SoftBank・Y!mobile両キャリアで**1つの「認証情報管理シート」を共用**する。
...
2. メニュー「自動同期タイマーを設定（初回のみ）」を実行 → 1分毎の自動同期トリガーを登録
3. メニュー「回線管理表から認証情報を更新」を実行 → 初回同期
4. 認証情報シートの各行にパスワードを手動入力

#### シート構成

| シート名 | 役割 |
|---|---|
| 設定 | PDF保存先フォルダ・対象月（両キャリア共通） |
| 認証情報 | 電話番号・パスワード・キャリア・PDFの種類・運用端末（GAS自動同期） |
| 回線管理表 | 電話番号・キャリア・解約済・運用端末を一元管理（GASが認証情報シートへ自動反映） |
| SoftBankリンク | SoftBank PDF月別リンク（GAS自動更新） |
| Ymobileリンク | Y!mobile PDF月別リンク（GAS自動更新） |

#### 認証情報シート（電話番号・パスワード管理）

| 電話番号 | パスワード | キャリア | PDFの種類 | 運用端末 |
|---|---|---|---|---|
| 090-XXXX-XXXX | パスワード | SoftBank | 電話番号別 | iPhoneAir |
| 080-XXXX-XXXX | パスワード | Ymobile | 電話番号別 | iPhone16 |

- ハイフンあり/なしどちらでもOK
- `キャリア` 列で SoftBank / Ymobile スクリプトが自動フィルタリング
- SoftBankは `電話番号別` / `一括` / `機種別` をカンマ区切りで複数指定可
- **電話番号・キャリア・運用端末 は GASが回線管理表から自動同期**（手動編集不要）
- パスワード・PDFの種類のみ手動入力（GASは保持して上書きしない）
...
回線管理表シートを更新すると約1分後に認証情報シートへ自動反映される。

- 解約済（`解約済` = TRUE）の行は認証情報シートから自動削除
- 新規行は認証情報シートに自動追加（パスワードは空欄なので手動入力）
- キャリア・運用端末は常に回線管理表の値で上書き
...
**SMS認証で端末名が表示されない**
認証情報管理シートの月別シートに「運用端末」列が設定されているか確認する。
