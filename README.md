...
ルートフォルダの `setup_spreadsheet.gs` を認証情報管理シートのApps Scriptに貼り付けて `setupSheet()` を実行すると、必要なシートが自動作成される。
...
| 認証情報 | 電話番号・パスワード・キャリア・PDFの種類 |
| SoftBankリンク | SoftBank PDF月別リンク（GAS自動更新） |
| Ymobileリンク | Y!mobile PDF月別リンク（GAS自動更新） |
| 1月, 2月 ... | 電話番号管理・解約済フィルタリング用月次データ |

#### 認証情報シート（電話番号・パスワード管理）

| 電話番号 | パスワード | キャリア | PDFの種類 |
|---|---|---|---|
| 090-XXXX-XXXX | パスワード | SoftBank | 電話番号別 |
| 080-XXXX-XXXX | パスワード | Ymobile | 電話番号別 |
...
月別シート（1月, 2月 ...）の `解約済` 列が `TRUE` のアカウントは処理をスキップする。対象月の月番号に一致するシート（"2月"等、末尾一致で"(2025)12月"等も対応）が自動参照される。
