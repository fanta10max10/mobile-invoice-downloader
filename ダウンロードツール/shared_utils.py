...
carrier_family による分岐:
  ...
  - "docomo": docomo（My docomo経由のPDFダウンロード）
"""
...
class CarrierConfig:
    ...
    carrier_family: str = "softbank"  # "softbank" / "au" / "docomo" — ログイン・PDF取得フローの分岐に使用
                    ...
                    if target_label in labels_in_block:
                        ...
                        if label_idx < len(amounts_in_block):
                            ...
                            if a.isdigit() and 0 < int(a) <= 9999999:
                                ...
                                return f"{int(a)}円(税抜)"
        elif "NTTドコモ" in text or "ドコモ" in text:
            # docomo: 「ご請求額」「ご利用金額」等から金額を取得
            # 適格請求書: 「ご請求額（税込）」パターン
            m_bill = re.search(r'ご請求額[（(]税込[）)]\s*([\d,]+)', text)
            if m_bill:
                a = m_bill.group(1).replace(',', '')
                if a.isdigit():
                    log.info(f"  PDF金額取得（ご請求額税込）: {a}円(税込)")
                    return f"{int(a)}円(税込)"
            # 「合計」パターン
            m_total = re.search(r'合計\s*([\d,]+)円', text)
            if m_total:
                a = m_total.group(1).replace(',', '')
                if a.isdigit():
                    log.info(f"  PDF金額取得（合計）: {a}円")
                    return f"{int(a)}円"
            # 「ご利用金額」パターン
            m_usage = re.search(r'ご利用金額\s*([\d,]+)', text)
            if m_usage:
                a = m_usage.group(1).replace(',', '')
                if a.isdigit():
                    log.info(f"  PDF金額取得（ご利用金額）: {a}円")
                    return f"{int(a)}円"
        else:
            # SoftBank/Ymobile: 「計」（個別回線の小計）をそのまま使用
            m_total = re.search(r'(?<![小合課])計\s*([\d,]+)', text)
            if m_total:
                a = m_total.group(1).replace(',', '')
                if a.isdigit():
                    log.info(f"  PDF金額取得（計）: {a}円(税抜)")
                    return f"{int(a)}円(税抜)"
        return ""
    except Exception as e:
...
def do_login_and_navigate(ctx: BillingContext, page, phone_number: str, password: str,
    ...
    if ctx.config.carrier_family == "docomo":
        return _do_docomo_login_and_navigate(ctx, page, phone_number, password)
...
def select_target_month(ctx: BillingContext, page, year: str, month: str) -> bool:
    ...
    if ctx.config.carrier_family == "docomo":
        return _docomo_select_target_month(ctx, page, year, month)
...
def download_pdf_from_page(
    ...
    if ctx.config.carrier_family == "docomo":
        return _docomo_download_pdf_from_page(ctx, page, save_dir, year, month, phone, pdf_types)
    if pdf_types is None:
        pdf_types = {"電話番号別"}
...
def download_billing_pdf(
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # au/docomo系はダウンロード関数内で月選択も行うためスキップ
            if ctx.config.carrier_family not in ("au", "docomo"):
...
def _do_docomo_login_and_navigate(ctx: BillingContext, page, phone_number: str, password: str) -> bool:
    ...
    login_id = ctx.phone_login_id_map.get(phone_number, phone_number)
    if login_id != phone_number:
        log.info(f"dアカウント ログイン情報を入力中...（dアカウントID: {login_id[:3]}***）")
    else:
        log.info("dアカウント ログイン情報を入力中...（dアカウントID未設定のため電話番号を使用）")
    ...
    try:
        ...
        log.error(f"  dアカウントログインフォームの操作に失敗: {e}")
        return False
    ...
    if _is_on_docomo_auth_page(ctx, page):
        ...
        for pattern in error_patterns:
            if pattern in page_text:
                log.error(f"  dアカウントログインエラー: {pattern}")
    ...
    if _is_on_docomo_auth_page(ctx, page):
        ...
        if "セキュリティコード" in page_text or "2段階認証" in page_text or "確認コード" in page_text:
            ...
            try:
                ...
                log.error("  dアカウント認証がタイムアウトしました")
                return False

    # Step 4: 料金ページに到達したか確認
    final_url = page.url
    log.info(f"  最終URL: {final_url}")

    if _is_on_docomo_auth_page(ctx, page):
        log.error("  dアカウント認証を完了できませんでした")
        return False
...
def _handle_docomo_2fa(ctx: BillingContext, page, phone_number: str) -> bool:
    ...
    for attempt in range(8):
        ...
        try:
            ...
            if trust_btn.first.is_visible(timeout=2000):
...
def _docomo_select_target_month(ctx: BillingContext, page, year: str, month: str) -> bool:
    """My docomoの料金ページで対象月を選択する。"""
    target_label = f"{int(year)}年{int(month)}月"
    log.info(f"対象月を選択中: {target_label} (docomo)")
    ...
    try:
        ...
        if selects.count() > 0:
            for i in range(selects.count()):
                ...
                try:
                    ...
                    log.info(f"  月を選択しました（セレクトボックス）: {int(month)}月")
                    page.wait_for_load_state("networkidle")
                    time.sleep(2)
                    return True
                except Exception:
        ...
        # パターン2: リンクまたはボタンで月を選択
        month_link = (
            page.get_by_text(target_label, exact=False)
            .or_(page.get_by_text(f"{int(month)}月ご利用", exact=False))
            .or_(page.get_by_text(f"{int(month)}月分", exact=False))
        )
        try:
            if month_link.first.is_visible(timeout=5000):
                ...
                page.wait_for_load_state("networkidle")
                time.sleep(2)
                log.info(f"  月を選択しました（リンク）: {target_label}")
                return True
        except Exception:
            pass

        # パターン3: 確定月リストからラジオボタン
        radios = page.locator('input[type="radio"]')
        for i in range(radios.count()):
            radio = radios.nth(i)
            value = radio.get_attribute("value") or ""
            ...
            try:
                ...
                pass
            if f"{year}{month}" in value or target_label in label_text:
                radio.check(force=True)
                log.info(f"  月を選択しました（ラジオ）: value={value}")
                time.sleep(1)
                return True

        log.error(f"  対象月 {target_label} の選択肢が見つかりません")
        return False
...
def _docomo_download_pdf_from_page(
    ctx: BillingContext, page, save_dir: Path, year: str, month: str, phone: str,
    pdf_types: set[str] | None = None,
) -> tuple[bool, list[str]]:
...
def _docomo_download_usage_detail(
    ...
    for attempt in range(3):
        try:
            ...
            log.info(f"  [利用内訳] 保存完了: {dest}")
            return final_name

        except Exception as e:
            if attempt < 2:
                log.warning(f"  [利用内訳] ダウンロード失敗 (試行{attempt+1}/3): {e}")
                time.sleep(2 * (2 ** attempt))
            else:
                log.warning(f"  [利用内訳] ダウンロード失敗 (全3回試行): {e}")
