...
| 電話番号 | パスワード | PDFの種類 | SMS送付先末尾4桁 |
|---|---|---|---|
| 090-XXXX-XXXX | パスワード | 電話番号別 | （任意） |

`SMS送付先末尾4桁` はファミリープランでログインする電話番号とSMS受信番号が異なる場合に設定する。例えばSMSが `090-1234-5678` に届くなら `5678` を入力。空欄の場合は電話番号の末尾4桁で自動選択。
...
| `ACCOUNT_INTERVAL` | `60` | アカウント間のインターバル（秒）。SMS連続送信のレート制限対策 |
